@extends('layouts.sdz.template-02')

@section('header')
	@if(Auth::check())
		<div class="btn-group pull-right">
			{!! link_to_route('sdz_post.create', 'Créer un article', [], ['class' => 'btn btn-info']) !!}
			...
			<div class="col-md-12">
					...
					<h1>{{ $post->titre }}
                        <div class="pull-right">
                            @foreach($post->tags as $tag)
                                {!! link_to('sdz_post/tag/' . $tag->tag_url, $tag->tag, ['class' => 'btn btn-xs btn-info']) !!}
                            @endforeach
                        </div>
                    </h1>
				</header>
                ...
                <section>
					...
					@if(Auth::check() and Auth::user()->admin)
						{!! Form::open(['method' => 'DELETE', 'route' => ['sdz_post.destroy', $post->id]]) !!}
